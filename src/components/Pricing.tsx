    ...
    <section className="p-6 md:p-12 lg:p-20 bg-primary border-b-2 lg:border-b-4 border-brand-dark">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 lg:mb-24 gap-4">
        ...
        <div className="flex flex-col mb-4">
          <span className="font-bold tracking-widest text-sm md:text-base uppercase pb-2 text-brand-dark">
            PRICING MODELS
          </span>
          <div className="h-1 w-full bg-brand-dark"></div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12 w-full max-w-7xl mx-auto items-end">
        ...
        {pricingData.map((tier) => (
          <div
            key={tier.id}
            className={`border-4 border-primary-text p-8 shadow-[7px_7px_0px_0px_rgba(26,26,26,1)] h-137 flex flex-col justify-between ${tier.isPopular ? "bg-secondary" : tier.badge === "enterprise" ? "bg-primary-text text-primary" : "bg-primary"}`}
          >
            <div>
              {tier?.isPopular ? (
              ...
              <h3 className="text-4xl font-bold mb-2 uppercase text-brand-dark tracking-tight">
                {tier.title}
              </h3>
              <div className="font-bold mb-10 text-brand-dark uppercase flex flex-col sm:flex-row sm:items-baseline">
              ...
              <ul className="space-y-6 mb-auto">
                {tier.features.map((feature, idx) => (
                  <li key={idx} className="flex items-center gap-4">
                    ...
                    <span className="font-bold text-sm uppercase tracking-wide text-brand-dark">
                      {feature}
                    </span>
                  </li>
                ))}
